...
def get_database_path() -> str:
    env_path = os.environ.get("PLA_DB")
    if env_path:
        return env_path
    return str(Path(__file__).resolve().parent / "pla.db")
...
def dashboard() -> Any:
    ...
    if latest_attempt:
        ...
        for row in topic_rows:
            if row["topic"] in latest_split and row["total"]:
                latest_split[row["topic"]] = round((row["correct"] / row["total"]) * 100, 2)
...
def admin_home() -> Any:
    ...
    counts = {
        ...
        "attempts": conn.execute("SELECT COUNT(*) FROM attempt").fetchone()[0],
        "responses": conn.execute("SELECT COUNT(*) FROM response").fetchone()[0],
        "attempts_with_responses": conn.execute(
            "SELECT COUNT(DISTINCT attempt_id) FROM response"
        ).fetchone()[0],
    }
